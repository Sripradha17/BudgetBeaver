...
1. **MongoDB Atlas connection string** — put it in the `.env` file at the project root:
   ...
   ```
   MONGODB_URI="mongodb+srv://<user>:<password>@<cluster-url>/budget-beaver?retryWrites=true&w=majority"
   PORT=5000
...
- **Wire them together.** Once the Render service is live, copy its URL (e.g.
  `https://budget-beaver-api.onrender.com`) and add it as a GitHub Actions secret named
  `VITE_API_URL`, with `/api` appended (**Settings → Secrets and variables → Actions →
  New repository secret**): `https://budget-beaver-api.onrender.com/api`. Push to `main` (or
