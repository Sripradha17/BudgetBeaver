...
export default function MoreSheet({ onClose, onLogout }) {
  ...
  return (
    <div id="more-sheet" className="fixed inset-0 z-40 lg:hidden" role="dialog" aria-modal="true" aria-label="More navigation">
      <div className="absolute inset-0 bg-ink/40 animate-backdrop-in" onClick={onClose} />
      <div id="more-sheet-panel" className="absolute inset-x-0 bottom-0 rounded-t-[1.75rem] bg-surface p-3 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-raised animate-modal-in">
